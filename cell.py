...
class RecommenderCell(tf.contrib.rnn.BasicLSTMCell):
    ...
    def __init__(self, num_units, batch_size, inp_D,
		forget_bias=1):
	...
	print(state.shape)
        _inputs_u, _inputs_i = tf.split(value=inputs, num_or_size_splits=2, axis=1)
        # shape(c):
        # shape(i): 
        c_u, h_u, c_i, h_i = tf.split(value=state, num_or_size_splits=4, axis=1)
		...
		with tf.variable_scope("Users"):
                    ...
                    # shape(concat):
                    concat_u = _linear([_inputs_u, h_u], 4 * self._num_units, True, 1.0)
		...
		with tf.variable_scope("Items"):
                    concat_i = _linear([_inputs_i, h_i], 4 * self._num_units, True, 1.0)
	...
	return tf.concat([_out_users,_out_items],1), tf.concat([new_c_u, new_h_u, new_c_i, new_h_i],1)
